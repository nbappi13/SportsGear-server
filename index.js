...
app.post("/addEquip", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ message: "Failed to add equipment", error: error.message });
  }
});
...
app.get("/equipment/home", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ message: "Error fetching equipment", error: error.message });
  }
});
...
app.get("/equipment/top-rated", async (req, res) => {
  try {
    const db = await getDB();
    const equipCollection = db.collection("equipments");
    const topRated = await equipCollection
      .find()
      .sort({ rating: -1 })
      .limit(6)
      .toArray();
...
app.get("/equipment/user/:email", async (req, res) => {
  try {
    ...
    const equipment = await equipCollection
...
app.delete("/equipment/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ message: "Error deleting equipment", error: error.message });
  }
});
...
app.post("/special-deals", async (req, res) => {
  try {
    ...
    const collectionExists = await db
      .listCollections({ name: "specialDeals" })
      .hasNext();
    if (!collectionExists) {
    ...
    res
      .status(500)
      .send({ message: "Failed to add special deal", error: error.message });
  }
});
...
app.get("/special-deals", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ message: "Error fetching special deals", error: error.message });
  }
});
...
app.get("/special-deals/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ message: "Error fetching special deal", error: error.message });
  }
});
...
app.delete("/special-deals/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ message: "Error deleting special deal", error: error.message });
  }
});
...
app.get("/testimonials", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ message: "Error fetching testimonials", error: error.message });
  }
});
